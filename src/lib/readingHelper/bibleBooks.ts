...
/**
 * 영어 책 이름. 통독 계획은 인터넷도 DB 도 없이 도는 순수 모듈이라, 성경 표의
 * name_en 을 여기서 읽을 수가 없다. 그래서 한국어 이름·장수와 마찬가지로 이
 * 목록에도 함께 적어 둔다.
 *
 * ⚠️ 성경 표(books.name_en)와 **같은 것이 두 군데** 있다. 66권 정경 목록이라
 *    바뀔 일이 없어 그대로 두지만, 한쪽을 고치면 다른 쪽도 봐야 한다.
 */
