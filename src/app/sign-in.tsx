...
import { Field, Hero, InvCard, PrimaryButton } from '@/features/inventory/ui';
import { useAuth } from '@/lib/auth';

/**
 * 로그인.
 *
 * 데이빗바이블·교회운영ON 과 **같은 계정**이다(같은 Supabase 프로젝트를 쓴다).
 * 그래서 「새로 가입하세요」를 앞세우지 않고, 쓰던 이메일로 들어오는 칸을
 * 먼저 둔다.
 */
    ...
    <ThemedView style={styles.container}>
      <SafeAreaView style={styles.safeArea} edges={['bottom']}>
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <InvCard>
            <ThemedText themeColor="textSecondary" style={Type.caption}>
              데이빗바이블·교회운영ON 을 쓰고 계시면 같은 이메일과 비밀번호로 그대로 들어오시면
              됩니다. 계정이 따로 필요하지 않습니다.
            </ThemedText>
          </InvCard>
