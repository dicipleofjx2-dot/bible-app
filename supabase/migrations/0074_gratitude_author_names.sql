...
--
-- ⚠️ `0073_gratitude_feed_no_fanout.sql` 과 **같은 함수**를 고친다. 나중에 도는
-- 쪽이 이긴다. 이 파일은 0073 이 고친 것(join 때문에 줄이 불어나는 문제)을 모두
-- 담고 있으므로, 둘 다 돌린다면 **0073 → 0074 순서**로 돌리면 되고, 이것 하나만
-- 돌려도 결과는 같다. 거꾸로 돌리면 아래의 이메일 문제가 되살아난다.
-- (다른 창에서 같은 자리를 동시에 고쳐 번호가 겹쳤다.)
--
