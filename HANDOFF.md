...
Last updated: **2026-09-11** (이메일 정리ON 1단계 + 사명기록관 1~3단계 + 인터뷰 녹음). Everything through `3175923` is **committed
...
## 이어서 (2026-09-11) — 이메일 정리ON 1단계(체험형 화면)

「이메일 정리ON」 기획서의 **1단계(§14 체험형 화면)** 를 앱 안에 넣었다.
**마이그레이션이 없다** — 실행할 SQL도, 만들 표도 없다. 기획서의 기술 구성은
Next.js 별도 서비스지만, 지금 있는 것은 분류·요약·정리 규칙을 사람이 직접
돌려 보는 체험판이라 이 앱 안에서 끝난다.

- **바깥 메일 서비스를 한 줄도 건드리지 않는다.** Gmail OAuth 도 IMAP 도 아직
  없다(§14 2·3단계). 화면에 보이는 것은 예시 메일 스무 통
  (`src/lib/emailOrganizerSample.ts`)이고, 「휴지통으로」를 눌러도 이 기기 안의
  표시만 바뀐다. 화면마다 그렇게 적어 두었다.
- **영구 삭제가 코드에 없다.** 가장 센 동작이 `trashed: true` 하나다(§3.1).
  이건 UI 를 잠근 것이 아니라 그런 함수를 만들지 않은 것이다.
- **보호는 세 겹이다**: ① 보호 분류(사역·재정·가족·일정·답장·문서)는 애초에
  삭제 후보가 되지 않고, ② 보호 발신자·보호 단어(계약·세금·보험·병원…)에
  걸리면 빠지고, ③ 답장이 필요하거나 중요도가 높거나 첨부가 있으면 또 빠진다.
  뺀 이유는 `guardReason` 으로 화면에 그대로 적는다. 검사에서 「보호 분류는
  어떤 설정에서도 삭제 후보가 되지 않는다」를 못으로 박아 두었다.
- **체험 모드(§3.3)** 는 안내 문구가 아니라 잠금이다. `canExecute()` 가 거짓인
  동안 휴지통 단추가 눌리지 않는다(기본 14일, 설정에서 직접 끝낼 수 있다).
- **언어모델을 부르지 않는다.** 메일 본문을 통째로 밖에 보내는 일이 §13.3·13.4
  와 부딪히고, 열쇠도 없다. 대신 §5·§6 의 판단 근거를 규칙으로 옮겼다
  (`src/lib/emailOrganizer.ts`). **요약은 본문에 실제로 있는 문장 셋을 고르는
  것**이 전부다 — 새 문장을 지으면 그 요약을 믿고 답장한 사람이 하지 않은 말을
  하게 된다.
- **답장은 초안까지만.** 발송 단추가 없는 것이 빠뜨린 것이 아니다(§7.5).
- **noreply 주소에는 「답장 필요」를 붙이지 않는다.** 택배사의 「수령 확인
  부탁드립니다」가 오늘 할 일로 올라오던 것을 검사에서 잡았다. 사람이 할 수
  없는 일이 오늘의 정리함에 쌓이면 그 화면을 안 믿게 된다.
- **삭제 후보는 발신자별로 묶어 보여 준다**(§8.3). 스무 줄을 하나씩 판단하게
  하면 결국 「전체 선택」을 누르고, 그때 섞인 메일이 함께 간다.
- **분류 판정을 저장하지 않는다.** 볼 때마다 다시 계산한다 — 규칙을 고쳤는데
  옛 판정이 남아 있으면 설정 화면이 거짓말이 된다. 스무 통에서는 공짜다.
- **작업 기록은 바뀌기 전 값을 함께 담는다**(§7.8). 되돌릴 때 무엇을 어떻게
  되돌릴지 나중에 추측하면, 사이에 다른 작업이 끼었을 때 엉뚱한 값으로 간다.
- 저장은 AsyncStorage 뿐이다. 메일 제목·발신자는 그 자체로 민감해서(§13.3)
  서버에 올릴 이유가 생기기 전에는 기기 밖으로 내보내지 않는다.
- 새 화면 다섯: `/email-organizer`(통합 받은편지함) · `today`(오늘의 정리함) ·
  `cleanup`(삭제 후보 검토 + 구독 정리) · `settings`(보호 규칙·활동 기록·되돌리기) ·
  `[id]`(상세 + 요약 + 분류 이유 + 답장 도우미). 들어가는 길: 성장 탭
  「📬 이메일 정리ON」.

**검증 (2026-09-11)**: `tsc --noEmit` 오류 0(이 리포의 기존 `@/global.css` 말고
새 오류 없음). 순수 함수를 노드로 돌려 **64가지를 대조** — 10개 분류 판정,
중요도 4단계, 보호 세 겹(설정을 바꿔 가며), 기간 설정이 실제로 후보를 바꾸는지,
요약 문장이 전부 원문에 있는지, 금액·날짜·시간·장소·사람·할 일 추출, 대화
잇기, 말투 네 가지 초안, 오늘의 정리함에 한 메일이 겹쳐 들어가지 않는지,
발신자 묶음과 구독 해지 제안, 체험 기간 잠금, 작업 취소가 최근 것부터
하나씩만 풀리는지 — 전부 예상대로(64/64). 검사 스크립트는 리포에 남기지
않았다(이 리포에 테스트 틀이 없다).

**사람이 봐 줘야 하는 것**: (1) 실제 브라우저·폰에서 다섯 화면이 제대로
그려지는지 — 이 환경에서는 번들 SQLite 가 안 열려 앱이 「저장소를 여는
중입니다…」에서 멈춘다(사명기록관 1단계 때와 같다), (2) 다크 모드,
(3) 분류 결과가 실제 목회 메일함의 감각과 맞는지(규칙 낱말은
`CATEGORY_RULES` 에서 바로 고칠 수 있다).

**아직 안 한 것(기획서 2~5단계)**: Gmail OAuth 실제 연동, 한메일 IMAP,
언어모델 요약·학습, 실제 발송, 구독 해지 실행, 일정·할 일 서비스 연동,
모바일 알림. 문체·톤 학습(사용자 수정 결과를 규칙으로 굳히는 것)도 여기 속한다
— 지금은 수정 결과를 기록만 하고 규칙으로 반영하지는 않는다.
...
## 이어서 (2026-09-08) — 사명기록관 (선교사·목회자 사역 전기 기록)

기획서(「사명기록관」 AI 사역 전기 제작 플랫폼)의 **1단계 MVP**를 이 앱 안에
넣었다. 마이그레이션 `0079_mission_archive.sql` 이 하나 더 있다 — **아직 실행
안 됐다면 화면은 열려도 아무것도 저장되지 않는다(표가 없다).**

- 표 넷: `mission_subjects`(사역자) · `mission_answers`(인터뷰 답) ·
  `mission_timeline`(연표) · `mission_chapters`(장별 초고).
- **정책은 owner_id = auth.uid() 하나뿐이다.** 공유 스위치도 관리자 예외도
  두지 않았다 — 보안 지역의 국가·도시·현지 성도 실명이 그대로 적히는 표다.
  「공개 범위」(visibility) 는 표를 남에게 여는 값이 아니라, **원고를 내보낼 때
  무엇을 빼는지**를 정하는 값이다(기획서 §14).
- **꼬리질문에 언어모델을 부르지 않는다.** 앱에 모델 열쇠가 없기도 하고,
  사역자의 생애 기록을 통째로 밖에 보내는 일이 §14 와 정면으로 부딪힌다.
  대신 §7 의 「몇 년도인가 · 어디인가 · 누구였는가 · 그 뒤로 무엇이 달라졌는가 ·
  근거가 있는가」를 **규칙**으로 옮겼다(`src/lib/missionArchive.ts`
  `followUps`). 모델보다 덜 똑똑하지만 빠뜨리지 않고 묻는다.
- **문체 변환(1인칭 자서전/3인칭 평전)은 넣지 않았다.** 규칙으로 한국어 문체를
  바꾸면 사역자의 목소리가 망가진다. 지금 원고 편집실이 하는 일은 **엮는 것**
  뿐이다 — 답한 그대로를 §12 의 15장 목차에 모으고, 근거(연도·장소·인물·확인
  상태)를 문단 끝에 붙이고, 연표를 부록으로 단다. 말하지 않은 사건은 만들지
  않는다(§21.5).
- **보안 모드**는 구조화된 칸(장소·인물)만 ○○ 로 가린다. 본문 속 지명까지
  기계가 지우려 들면 반드시 놓친다 — 사람이 다치는 쪽이라 흉내내지 않고,
  원고 맨 앞에 「본문은 사람이 직접 확인할 것」을 적는다.
- **연도 없는 연표 줄은 받지 않는다.** 짐작한 연도를 넣는 순간 그 짐작이
  그대로 교회사가 된다. 달은 비워 둘 수 있다.
- 답은 `(subject_id, question_key)` 유일 제약 + upsert 다. 「이어서 말하기」는
  새 답이 아니라 그 답을 고치는 일이라, 두 걸음으로 나누면 끊길 때 같은 질문에
  답이 둘 남는다.
- 들어가는 길: 성장 탭 「🕯️ 사명기록관」. 화면은 `/mission-archive`(사역자 목록)
  → `/mission-archive/[id]`(그 사람의 기록 홈) → `interview` · `timeline` ·
  `manuscript`. 글자 크기를 앱 기본보다 한 단계 키웠다 — 쓰는 분이 대개 은퇴를
  앞둔 사역자다(§16).

**검증 (2026-09-08)**: `tsc --noEmit` 통과(이 리포의 유일한 기존 오류
`@/global.css` 말고 새 오류 없음). `lib/missionArchive.ts` 를 노드로 돌려 40여
가지를 대조 — 역할별 질문 걸러내기(목회자에게 선교지 언어를 묻지 않는지),
§7 예시 문장(「교회 문을 닫으려던 때 한 성도가 찾아왔습니다」)에 연도·인물·변화
꼬리질문이 붙는지, 이미 채운 칸은 다시 안 묻는지, 비공개가 어떤 원고에도 안
들어가는지, 공개용에서 교회내부가 빠지는지, 보안 모드에서 장소·인물이 가려지는지,
연도 없는 답이 연표로 안 가는지, 15장 목차가 36문항을 하나도 빠뜨리지 않는지 —
전부 예상대로. 라우터에 새 화면 다섯이 등록된 것은 `/_sitemap` 으로 확인했다.

**사람이 봐 줘야 하는 것**: (1) `0079_mission_archive.sql` 실행, (2) 로그인
상태의 왕복(사역자 등록 → 인터뷰 답 저장 → 연표 → 원고 내보내기), (3) 이 세션의
브라우저에서는 앱이 「저장소를 여는 중입니다…」에서 멈춰(번들 SQLite 가 이
환경에서 안 열린다) **새 화면을 눈으로 못 봤다** — 실제 브라우저·폰에서 화면이
제대로 그려지는지, (4) 다크 모드.

**아직 안 한 것(기획서 2·3단계)**: 음성 녹음·자동 녹취, 설교·사진·선교 편지
자료실, 가족·동역자 원격 증언 링크, 선교 지도, 다큐 대본, 디지털 기념관.
문체 변환과 EPUB/PDF 출력도 여기 속한다(지금은 Markdown 복사·내려받기).

## 이어서 (2026-09-08, 2단계) — 사명기록관 사역 아카이브

기획서 2단계(자료실·공동 증언·지도·사실 검토·전자책)를 얹었다. 마이그레이션
`0080_mission_archive_stage2.sql` — **0079 다음에 실행한다. 실행 전에는 자료실과
증언 화면이 열려도 아무것도 담기지 않는다.**

- **자료 통만 비공개다.** 이 앱의 다른 통(열매 사진·말씀카드 배경)은 공개
  읽기지만, 여기엔 선교 편지·파송장·현지 성도 얼굴이 들어간다. 주소가 한 번
  새면 되돌릴 길이 없어 `public: false` 로 만들고 화면은 **한 시간짜리 서명
  주소**를 매번 받아 연다(`assetSignedUrl`).
- **증언자는 계정을 만들지 않는다.** 표는 잠근 채로 두고 `security definer`
  함수 둘만 열었다 — `mission_witness_prompt`(물어볼 것만 돌려줌) ·
  `mission_submit_testimony`(넣기만 함). **증언자는 자기가 낸 답조차 다시 읽지
  못한다.** 링크가 남의 손에 넘어가도 읽을 것이 없어야 하기 때문이다. 토큰은
  `gen_random_bytes(18)`, 사역자가 언제든 닫을 수 있고(active) 기한도 둔다.
  한 번에 스무 개까지만 받는다.
- **`/witness/[token]` 만은 성경 저장소를 열지 않고 그린다**
  (`_layout.tsx` 맨 앞 갈림길). 증언 한 편 남기러 온 가족에게 수십 MB 성경
  데이터베이스를 내려받게 하고 탭 자리다툼(DbTabGate)까지 지나게 할 이유가
  없다. 평소 경로는 한 줄도 달라지지 않는다.
- **서로 다른 기억을 합치지 않는다**(§10). 원고 부록에 이름·관계를 달아 나란히
  싣고, 같은 질문에 증언이 둘 이상이면 「합치지 않았다」고 적는다.
- **「반복해서 나온 말」은 낱말 세기다.** 「핵심 메시지를 찾는다」고 하지
  않는다 — 형태소 분석기 없이 자주 쓰는 조사만 꼬리에서 떼어 어림한 것이라,
  세어 놓고 사역자에게 되묻는 형태로 뒀다.
- **지도는 밖에서 연다**(성경지도와 같은 판단). 장소별로 연표·인터뷰·자료를
  모아 보여 주고 지도만 링크로 넘긴다. **보안 모드면 지도 단추를 감춘다** —
  지명을 지도 검색에 넘기는 것만으로도 그 지역이 밖에 남는다.
- **EPUB 대신 인쇄용 HTML 한 장.** EPUB 은 zip 이라 압축 라이브러리를 새로
  실어야 하는데, 브라우저 인쇄(→ PDF 저장)로 종이책 원고와 전자책이 둘 다
  나온다. A5 쪽 크기, 장마다 쪽 나눔. 먼저 쓰이는지 보고 필요하면 그때 EPUB.
- **어두운 모드 단추 글자색을 고쳤다**(`features/mission/ui.tsx`). 어두운
  모드의 accent(#EC9A70)에 흰 글자는 2:1도 안 나온다. 밝은 바탕엔 어두운 글자를
  얹는다 — TreeCanvas 의 열매 글자와 같은 방식. (다른 화면들은 여전히 흰
  글자다. 앱 전체를 손보는 것은 따로 볼 일이다.)
- 새 화면: `assets`(자료실) · `witnesses`(증언 요청·수신) · `places`(발자취) ·
  `review`(사실 검토실), 그리고 로그인 없이 여는 `/witness/[token]`.

**검증 (2026-09-08)**: `tsc --noEmit` 오류 0. 2단계 순수 함수도 노드로 대조 —
조사를 뗀 낱말 세기(은혜가/은혜를/은혜는 → 은혜 3), 장소 묶기(첫 해·끝 해·자료
수), 원고 부록(엇갈린 증언 둘 다 실림, 비공개 증언 빠짐, 보안 모드에서 증언자
이름 가림), 인쇄용 HTML(쪽 나눔·표 머리·꺾쇠 이스케이프) — 전부 예상대로.
**증언 화면은 실제 브라우저에서 눈으로 확인했다**(이 화면만 저장소 밖이라
이 환경에서도 뜬다): 밝은/어두운 모드 스크린샷, 콘솔 에러 0, 그리고 서버 함수를
가짜로 채워 **성함·관계·답을 적고 「보내기」까지 눌러** 실제로 나가는 값이
`mission_submit_testimony` 의 인자 모양과 맞는 것까지 확인했다.

**사람이 봐 줘야 하는 것**: (1) `0080` 실행(0079 먼저), (2) 로그인 상태의
자료 올리기·서명 주소로 열기, (3) 진짜 기기에서 증언 링크 왕복, (4) 인쇄
창(팝업 차단), (5) 나머지 새 화면들의 다크 모드.

## 이어서 (2026-09-10, 3단계) — 디지털 기념관 + 다큐 대본

마이그레이션 `0081_mission_memorial.sql` — **0079·0080 다음에 실행한다.**

- **기념관은 이 앱에서 밖으로 열리는 유일한 문이다.** 0079·0080 의 표는 전부
  본인만 읽는데, 여기만 예외다. 그래서 문을 좁게 깎았다: ① 사역자가 직접 켜야
  열리고(published), ② 「공개」로 표시한 것만 나가고(기본값이 「교회 내부」라
  **손대지 않으면 기념관은 비어 있다** — 실수로 새는 쪽이 아니라 실수로 비는
  쪽으로 기울였다), ③ 읽기는 함수 하나(`mission_memorial`)뿐이며, ④ **보안
  모드면 아예 열리지 않는다**(§21.8 — 사람이 출판보다 앞선다).
- **거르는 일은 서버 함수 안에서 한다.** 화면에서 거르면 화면을 고칠 때마다
  새어 나갈 자리가 생긴다. 공개 화면은 받은 것을 그리기만 한다.
- **기념관 사진은 통이 다르다.** 0080 의 자료 통은 비공개(서명 주소)라 익명
  방문자가 못 연다. 그래서 기념관에 걸 사진은 **사역자가 한 번 더 누른 것만**
  공개 통(`mission-memorial-photos`)으로 복사한다(`publishPhotoToMemorial`).
- **다큐 스튜디오는 영상을 자르지 않는다** — 자를 영상이 이 앱에 없다. 만드는
  것은 대본이다. 씬마다 화면·내레이션·사역자 육성·증언을 **한 문단으로 녹이지
  않고 나란히** 둔다: 편집하는 사람이 어디까지가 본인 말이고 어디부터 우리가
  붙인 말인지 봐야 한다.
- **내레이션은 연표에 적힌 사실만으로 짓는다.** 「그는 두려웠다」는 아무도 한
  적 없는 말이다. 형용사가 필요하면 사람이 쓴다.
- 하이라이트(§18 3단계 「영상 인터뷰 하이라이트 추출」)는 **어느 답이 장면이
  되는지**를 고르는 것으로 옮겼다. 잣대는 연도·길이·전환 낱말·기적 증언·자료
  유무이고, **고른 이유를 함께 보여 줘** 사람이 뒤집을 수 있다.
- 공개 기념관(`/memorial/[slug]`)도 증언 화면과 같이 **성경 저장소 밖에서**
  그린다(`_layout.tsx` 의 같은 갈림길에 경로를 하나 더했다).
- 주소(slug)는 영문 소문자·숫자·붙임표만 받는다. 한글 이름만 적으면 `slugify`
  가 빈 값을 돌려주고 화면이 되묻는다.
- 새 화면: `memorial`(기념관 관리) · `documentary`(다큐 대본), 그리고 로그인
  없이 여는 `/memorial/[slug]`.

**검증 (2026-09-10)**: `tsc --noEmit` 오류 0. 3단계 순수 함수 노드 검사 —
하이라이트 고르기(전환·기적은 뽑고 밍밍한 답과 비공개는 빼는지), 대본(연표
사실로 지은 내레이션, 연도 맞는 사진이 화면 지시로, 육성 원문 인용, 증언 자막,
범위 밖 이야기 제외, 고인/생존의 말투 차이, 보안 모드 지명 가림), 기념관 주소
검사와 준비 안내 — 전부 예상대로. **공개 기념관은 실제 브라우저에서 확인했다**:
서버 함수를 가짜로 채워 열린 기념관(연표·이야기·증언이 다 뜸)과 닫힌 기념관
(「아직 열리지 않은 기념관입니다」) 두 갈래를 그려 스크린샷으로 봤고 콘솔
에러 0.

**사람이 봐 줘야 하는 것**: (1) `0081` 실행, (2) 로그인 상태에서 기념관 켜고
사진 걸기 → 다른 기기·시크릿 창에서 그 주소가 실제로 열리는지, (3) **공개
범위를 바꾸지 않은 채 기념관을 켜 보고 정말로 비어 있는지**(이 설계의 핵심),
(4) 보안 모드를 켠 사역자의 기념관이 열리지 않는지, (5) 다큐 대본 인쇄 창.

**아직 안 한 것**: 음성 녹음·자동 녹취(STT 열쇠 없음), 다국어 자막·번역,
사진 기반 영상 생성, EPUB. 문체 변환(1인칭/3인칭)도 그대로 남아 있다.

## 이어서 (2026-09-11) — 사명기록관 인터뷰 녹음

사용자가 라이브에서 바로 찾아낸 구멍: **인터뷰에 녹음이 없었다.** 1단계에서
「STT 열쇠가 없다」는 이유로 미뤄 뒀는데, 잘못된 묶음이었다 — 녹음과 받아쓰기는
서로 다른 것이고 **녹음은 열쇠 없이 된다**(`expo-audio` 는 이미 깔려 있었다).
마이그레이션 `0082_mission_answer_audio.sql`(칼럼 둘 추가).

- **은퇴를 앞둔 분에게 한 시간짜리 이야기를 타자로 치게 할 수 없다.** 말로 하면
  한 시간 할 이야기를 글로는 한 문단 쓰고 만다. 이것이 이 기능의 전부다.
- **받아쓰기는 브라우저 것을 쓴다**(`SpeechRecognition`, `src/lib/dictation.web.ts`).
  열쇠도 돈도 새 묶음도 필요 없고, 무엇보다 **사역자의 생애 이야기를 외부 받아쓰기
  서비스로 통째로 보내지 않아도 된다**(0079 머리말의 §14 판단과 같은 줄기).
  앱(네이티브)에는 아직 없다 — `dictation.ts` 가 `supported: false` 로 조용히
  물러서고 **녹음은 그대로 된다.** 받아쓰기는 덤이지 필수가 아니다.
- 크롬은 몇 초만 조용해도 인식을 혼자 끝낸다. 인터뷰는 말과 말 사이가 길어서,
  사용자가 멈추지 않았으면 `onend` 에서 **다시 켠다**.
- **원본 음성은 지우지 않는다**(§21.9). 받아쓴 글이 틀리면 글을 고치지, 목소리를
  버리지 않는다 — 목소리 자체가 손주에게 남길 유산이다. 화면 문구도 그렇게 적었다.
- **멈추면 그 자리에서 저장한다.** 저장 단추를 안 누르고 화면을 나가면 그 한
  시간이 사라지기 때문이다.
- **「답했다」의 뜻을 넓혔다**(`answered()`): 녹음만 남겨도 답한 것이다. 안 그러면
  한 시간을 말하고도 진행이 0 으로 남는다. 원고에서는 그런 답을 「녹음이 있습니다.
  아직 글로 옮기지 않았습니다」로 적어 자리를 비워 둔다 — 빠뜨리면 말한 적 없는
  이야기가 된다.
- 파일은 새 통을 만들지 않고 **0080 의 비공개 통**에 담는다. 성격이 같은 파일이고,
  통이 늘면 정책이 늘고 정책이 늘면 새는 구멍이 는다.
- **파일 종류를 주소에서 짐작하지 않는다.** 웹 녹음 주소는 `blob:...` 이라
  확장자가 없어서 처음엔 전부 `.m4a` 로 올라갔다(속은 webm). 응답의
  `content-type` 에 직접 물어보도록 고쳤다.
- 보안 모드 사역자에게는 「받아쓰기를 끄고 녹음만 남기시라」고 화면에 적는다.
- `app.json` 의 `expo-audio` 플러그인에 한국어 마이크 권한 문구를 넣었다.
  **앱에서 쓰려면 APK 를 다시 구워야 한다**(웹은 배포만 하면 된다).

**검증 (2026-09-11)**: `tsc --noEmit` 오류 0. **실제 브라우저에서 왕복을 돌렸다** —
가짜 마이크를 물린 Chromium 으로 인터뷰 화면을 열고 「녹음 시작」→ 4초 →
「멈추고 저장하기」까지 눌러, 녹음 중 시계(00:03)와 붉은 테두리, 멈춘 뒤
「녹음을 저장했습니다」, 올라간 파일(54KB), 표에 저장된 `audio_path`/
`audio_seconds`(4초), 「들어보기」 단추가 생기는 것까지 확인했다(콘솔 에러 0).

**사람이 봐 줘야 하는 것**: (1) `0082` 실행, (2) 진짜 폰·브라우저에서 한국어
받아쓰기가 쓸 만한지(크롬 기준), (3) 긴 녹음(10분 이상) 업로드, (4) 앱에서
녹음하려면 EAS 빌드 한 번.

## 이어서 (2026-09-04) — 나무 그림 다시 그리기 + R2M 개인기도 훈련 연동

마이그레이션 `0077_prayer_tree_training.sql` 이 하나 더 있다. **아직 실행 안 됐다면
「이 사람을 위해 기도합니다」 단추가 동작하지 않는다.**

**나무 그림** (`src/lib/treeShape.ts` 새 파일)
- 타원 여섯 개로 그리던 것을 **재귀로 뻗는 가지**로 바꿨다. 씨앗을 고정해
  (기본 42) 언제나 같은 나무가 나온다 — 모양이 매번 달라지면 열매를 놓아 둔
  자리가 다른 가지 위에 뜬다.
- **가지는 다섯 대까지만.** 여섯 대로 두었더니 잎이 3,159개가 됐다.
  react-native-svg 는 도형 하나가 네이티브 뷰 하나라 폰에서 버벅인다. 지금은
  가지 165 · 잎 541.
- 갈라질 때마다 각도를 **위로 조금 되돌린다**. 이게 없으면 옆으로만 퍼져
  나무가 아니라 울타리처럼 보인다(처음 그린 것이 그랬다).
- 다 자란 뒤 **한 번 재서 화면에 맞춰 넣는다**(`FIT`). 난수가 어디까지 뻗을지
  정하므로, 이게 없으면 씨앗에 따라 잎이 화면 밖으로 잘린다. 씨앗 4종으로
  전부 화면 안에 들어오는 것을 확인했다.
- 잎 색은 난수가 아니라 **왼쪽 위에서 오는 빛**으로 정한다. 난수로 흩뿌리면
  평평한 모자이크가 된다.
- 씨앗 42 는 열 개를 견줘 왕관이 가장 둥글고(가로:세로 2.29) 열매 놓을 가지
  끝이 가장 많이(10곳) 나온 것이다. 새 열매의 기본 자리도 이 가지 끝을 쓰고,
  **이미 고른 자리에서 가장 먼 것**을 하나씩 집어 한쪽 어깨에 몰리지 않게 한다.
- 나무 폭은 창 너비 −16px(최대 620) 로 넓혔다.

**R2M 개인기도 훈련 연동** (0077)
- 기도카드에 「🙏 이 사람을 위해 기도합니다」. 누르면 **서버 함수 하나**
  (`pray_for_fruit`) 안에서 열매의 마지막 기도 시각·횟수와 R2M `prayer_logs`
  가 같이 올라간다. 화면에서 둘을 따로 부르면 사이에서 끊길 때 한쪽만 남는다.
- **prayer_logs 는 하루 한 줄만.** 성장기록이 줄 수를 「기도한 날」로 세기
  때문이다. 날의 경계는 **서울 기준** — UTC 자정으로 끊으면 새벽기도가 어제
  것이 된다.
- 「오늘의 훈련」의 기도 항목이 `/prayer-group`(샬롬기도단) 대신
  **`/prayer-tree`** 로 간다. 샬롬기도단은 함께 나누는 자리라 그대로 두었고,
  거기서 「오늘 기도했어요」를 눌러도 여전히 같은 항목이 채워진다.
- 나무 화면 위에 오늘의 훈련 띠(기도 ✓ / 아직), 오늘 기도한 열매는 이름 앞에
  🙏 — 나무만 보고 「오늘 어디까지 돌았는지」가 읽힌다.
- `backgroundSelected` 는 이 테마에서 `accent` 와 **같은 색**이다. 「오늘
  기도함」 상태를 그 색으로 두었더니 안 누른 단추와 구분이 안 되고 글자
  대비도 나빴다 — `accentSoft` 바탕 + `accent` 글자로 물러섰다.

**검증 (2026-09-04)**: `tsc --noEmit` 통과. `buildTree` 를 노드로 돌려
같은 씨앗이 같은 나무를 내는지, 씨앗 4종이 모두 화면 안에 들어오는지, 도형
수와 열매 자리 수를 셌다. Playwright(Chromium)로 나무·기도카드·나무 화면을
그려 눈으로 확인(콘솔 에러 0). **로그인 왕복은 여전히 사람이 봐 줘야 한다** —
특히 「기도합니다」를 눌렀을 때 R2M 대시보드의 기도 ✓ 가 켜지는지.
