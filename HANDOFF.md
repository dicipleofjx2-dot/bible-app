...
Last updated: **2026-09-04** (책갈피 절 번호 작업은 미커밋). Everything through `3175923` is **committed
...
## 이어서 (2026-09-04, 셋째) — 과일상자

마이그레이션 `0078_prayer_fruit_harvest.sql`. **이것을 실행하기 전에 배포하면
나무 화면이 아예 안 뜬다** — 화면이 `harvested_at` 을 함께 읽는데 그 칸이 없으면
PostgREST 가 통째로 거절한다. 순서를 지킬 것: 마이그레이션 먼저, 배포는 그 다음.

- **다 익은 열매(기도제목이 모두 응답된 열매)를 따서 상자에 담는다.**
  `prayer_fruits.harvested_at` 하나로 나무에 있는지 상자에 있는지를 가른다.
- **자동으로 따지 않는다.** 응답 체크 한 번에 열매가 사라지면 잘못 누른 사람은
  기록을 잃은 줄 안다. 기도카드의 「🧺 열매를 따서 과일상자에 담기」 한 걸음을
  거친다. 되돌리기는 「🌳 나무에 다시 걸기」.
- **기도제목을 새로 넣으면 조용히 나무로 돌아온다.** 더 이상 다 익은 열매가
  아니기 때문이다. 상자에 있는데 미응답 제목이 달려 있는 것이 제일 이상하다.
- 자리(`pos_x`/`pos_y`)는 담는 동안에도 **건드리지 않는다** — 다시 걸면 원래
  가지로 돌아간다.
- 상자는 **아래 줄부터** 채운다(`FruitBox`). flexWrap 에 맡기면 위 줄부터 차서
  새 열매가 아래로 밀린다 — 「쌓인다」로 안 읽힌다. 줄을 손으로 나눠 뒤집었다.
- 넓은 화면(≥760px)이면 나무 **옆**에, 좁으면 나무 **아래**에 둔다. 옆에 세울
  때는 나무 폭에서 상자 몫(206px)을 덜어 낸다.
- 가꾸기 화면에서도 딴 열매는 나무에 안 그린다. 상자에 담긴 열매를 고르고
  나무를 누르면 그 자리로 **다시 걸린다**.

**검증 (2026-09-04)**: `tsc --noEmit` 통과. Playwright 로 넓은 화면(상자 7개가
아래 4 · 위 3 으로 쌓이는지) · 좁은 화면(빈 상자와 안내 문구) · 다 익은 열매의
기도카드(담기 단추) 를 그려 확인, 콘솔 에러 0.
