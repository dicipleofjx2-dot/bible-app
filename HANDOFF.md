...
- **옛 파일이 서버보다 뒤처져 있다**(이 세션에서 확인). 창세기·출애굽기·
  ...
  **169곳** 있는데 서버와 영어판은 객관식이다(`check-content.js` 주석이 말하는
  그 169개와 같은 수다). **그 파일들을 다시 올리면 서버가 되돌아간다** — 업로드
  스크립트가 그 앞에서 멈춰 선다(`ALLOW_QUESTION_DOWNGRADE`).
- **고치는 스크립트를 만들었다**: `scripts/sync-questions-from-server.js`.
  방향이 업로드와 **반대**다 — 서버를 기준으로 삼아 파일을 맞춘다.

  ```
  node scripts/sync-questions-from-server.js            # 무엇이 바뀌는지만 본다
  node scripts/sync-questions-from-server.js --write    # 실제로 고친다
  ```

  이 세션에서는 키가 없어 **실제 서버로는 못 돌렸다.** 대신 가짜 응답으로
  세 가지를 확인했다 — (1) 서버와 같을 때 `--write` 를 해도 187장이 **바이트까지
  그대로**, (2) 단답형 169개를 객관식으로 바꾼 가짜 응답에 대해 7개 파일의 그
  문항들만 갈아 끼워지고 영어 파일은 바이트 그대로, (3) 고친 뒤 다시 돌리면
  「고칠 것이 없습니다」. 옛 파일은 줄바꿈이 제멋대로라(같은 파일에서 폭 108 이
  인라인, 폭 55 에서 줄바꿈) **파일을 다시 찍지 않고 바뀌는 문항 덩어리만**
  갈아 끼운다 — 안 그러면 손대지 않은 수천 줄이 diff 에 섞인다.
- `memoryVerse` 와 요약(summary)은 이 스크립트가 건드리지 않는다. 요약이
  서버와 다르면 **알려 주기만 한다** — 어느 쪽이 최신인지 이력으로 알 수 없다.
